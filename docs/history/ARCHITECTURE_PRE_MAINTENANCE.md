# Archived Pre-Maintenance Architecture

> Non-authoritative historical snapshot captured at the maintenance-mode documentation refactor on 2026-09-10. The content below is the former monolithic `docs/ARCHITECTURE.md` preserved intact before its current contracts were routed into domain files. Current architecture is owned by `docs/ARCHITECTURE.md` and `docs/architecture/`.
