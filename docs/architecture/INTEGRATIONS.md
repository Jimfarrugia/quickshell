...
| AI provider quota | usage endpoints do not provide a local event source; system resume is a separate logind event | 5 minutes while a bar or dashboard consumer exists; resume and dashboard opening request a cycle when any provider's last attempt is overdue | one singleton adapter/poller and resume watcher shared by all monitors; stop at zero consumers | sequential provider requests remain one logical pending cycle, bounded response/auth reads, due checks and adapter coalescing prevent duplicate automatic cycles, manual retry of QE-generated timeout/network backoff, provider backoff with `Retry-After` respected; pre-sleep cancellation prevents false failures | retain last-known values and mark stale after 15 minutes; dashboard opening recalculates age immediately and the one-minute local age refresh does not contact providers | AI quota validation |
